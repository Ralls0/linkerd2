...
func Main(args []string) {
	...
	if *enableEndpointSlices {
		k8sAPI, err = k8s.InitializeAPI(
			...
			*kubeConfigPath,
			true,
			k8s.Endpoint, k8s.ES, k8s.Pod, k8s.RS, k8s.Svc, k8s.SP, k8s.TS, k8s.Job, k8s.NS, k8s.Node, k8s.CM,
		)
	} else {
		k8sAPI, err = k8s.InitializeAPI(
			ctx,
			*kubeConfigPath,
			true,
			k8s.Endpoint, k8s.Pod, k8s.RS, k8s.Svc, k8s.SP, k8s.TS, k8s.Job, k8s.NS, k8s.Node, k8s.CM,
		)
	}
	if err != nil {
